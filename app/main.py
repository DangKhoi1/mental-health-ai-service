...
def _needs_actionable_recommendations(message: str, bot_reply: str = "") -> bool:
...
def _build_default_recommendations(message: str) -> list:
    msg = (message or "").lower()
    ...
    if "mất ngủ" in msg or "ngủ" in msg:
        return [
            {
                "category": "SLEEP",
                "content": "Thiết lập giờ đi ngủ cố định trong 7 ngày liên tiếp, kể cả cuối tuần.",
            },
            {
                "category": "RELAXATION",
                "content": "Trước khi ngủ 20 phút, tắt màn hình và nghe nhạc không lời nhịp chậm.",
            },
            {
                "category": "BREATHING",
                "content": "Thực hành nhịp thở 4-4-6 trong 3 phút để hạ mức căng thẳng trước khi lên giường.",
            },
        ]
    ...
    if "stress" in msg or "căng thẳng" in msg or "áp lực" in msg:
        return [
            {
                "category": "BREATHING",
                "content": "Dừng lại 2 phút, hít vào 4 giây - giữ 4 giây - thở ra 6 giây, lặp lại 6 vòng.",
            },
            {
                "category": "EXERCISE",
                "content": "Đi bộ nhanh 10-15 phút để giải phóng năng lượng căng thẳng và làm dịu suy nghĩ.",
            },
            {
                "category": "JOURNALING",
                "content": "Viết ra 3 việc khiến bạn lo nhất, sau đó đánh dấu 1 việc có thể xử lý ngay hôm nay.",
            },
        ]
    ...
    return [
        {
            "category": "JOURNALING",
            "content": "Mỗi tối ghi 3 dòng: hôm nay mình thấy gì, điều gì làm mình mệt, điều gì làm mình biết ơn.",
        },
        {
            "category": "EXERCISE",
            "content": "Duy trì vận động nhẹ 15-20 phút mỗi ngày để ổn định tâm trạng và năng lượng.",
        },
        {
            "category": "SOCIAL",
            "content": "Kết nối với một người bạn tin cậy ít nhất 1 lần/tuần để chia sẻ cảm xúc.",
        },
    ]
...
Bạn rất dũng cảm khi đối diện với cảm xúc của mình. Bạn có muốn mình cung cấp một số thông tin liên hệ chuyên gia hoặc tổ chức hỗ trợ không?"""
            ...
            try:
                ...
                if len(recommendations) == 0 and _needs_actionable_recommendations(request.message, bot_reply):
                    recommendations = _build_default_recommendations(request.message)
